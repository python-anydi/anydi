...
from anydi._types import NOT_SET, build_typevar_map, resolve_typevars, to_list
...
def test_resolve_generic_type() -> None:
    typevar_map = {T: User}
    result = resolve_typevars(Repository[T], typevar_map)  # type: ignore
...
def test_resolve_annotated_type() -> None:
    V = TypeVar("V")
    typevar_map = {V: User}
    result = resolve_typevars(Annotated[V, "some_metadata"], typevar_map)  # type: ignore
...
def test_resolve_nested_generic() -> None:
    V = TypeVar("V")
    typevar_map = {V: User}
    result = resolve_typevars(list[Repository[V]], typevar_map)  # type: ignore
...
def test_resolve_union_with_nested_generic() -> None:
    ...
    result = resolve_typevars(Repository[V] | None, typevar_map)  # type: ignore
